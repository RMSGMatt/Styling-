...
export default function App() {
  ...
  const runAllKpiUpdates = async (urlsOverride, skuOverride) => {
    ...
    try {
      ...
      // - BackorderRate = BackorderVolume / totalDemand (due-date demand only)
      // - InventoryTurns = annualized customer shipments / avg inventory (requires avgInventoryNum + invUniqueDays captured in inventory block)
      if (urls.flow_output_file_url) {
        ...
        const shippedKey = pickFirstKey(sample, ["flow", "quantity", "shipped"]) || "flow";
        ...
        const totalDemand = dueRows.reduce((s, r) => s + toNum(r.demand), 0);

        // On-time day = backlog_out == 0 (demand fulfilled in full on due date)
        const EPS = 1e-9;
        const onTimeDays = dueRows.filter((r) => Math.abs(toNum(r.backlogOut)) <= EPS).length;
        ...
        // For inventoryTurns: annualize customer shipments (use dueRows if you prefer "due-date shipped", or shipRows for all shipped)
        const totalCustomerShippedUnits = dueRows.reduce((sum, r) => sum + toNum(r.shipped), 0);

        // Inventory Turns = annualized customer shipments / avg inventory
        if (typeof avgInventoryNum !== "undefined" && avgInventoryNum > 0 && typeof invUniqueDays !== "undefined" && invUniqueDays > 0) {
          const annualFactor = 365 / Math.max(invUniqueDays, 1);
          const annualizedThroughput = totalCustomerShippedUnits * annualFactor;
