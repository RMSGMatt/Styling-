      ...
      >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
          ...
          <div className="bg-slate-900/50 border border-slate-700/80 rounded-xl p-3">
            ...
            <p className="text-xl font-bold" style={{ color: "#9CF700" }}>
                  ...
                  <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
                    ...
                    <div className="rounded-xl border border-slate-700/70 bg-slate-900/50 p-3">
                      ...
                      <p className="text-lg font-semibold text-orange-300">
                        {execPeakBacklog.toLocaleString()}
                      </p>
                      <p className="text-[10px] text-slate-400 mt-1">
...
<section
  ...
  style={{
    background: "linear-gradient(160deg, rgba(24,7,7,0.96), rgba(34,10,10,0.96))",
    borderColor: "rgba(127,29,29,0.65)",
  }}
>
  <div className="flex items-center justify-between mb-3">
    <div>
      <p className="text-[11px] uppercase tracking-[0.22em] text-red-300/80 mb-1">
        Decision Narrative
      </p>
    ...
    <div className="text-right">
      ...
      <p className="text-xs font-semibold text-red-300">
        {execOnTimePct < 60 ? "High Service Risk" : execOnTimePct < 90 ? "Service Under Stress" : "Stable"}
      </p>
    </div>
  </div>
  ...
  <div className="grid grid-cols-2 xl:grid-cols-4 gap-3 mb-4">
    <div className="rounded-xl border border-red-900/40 bg-black/20 p-3">
      <p className="text-[11px] uppercase tracking-wide text-slate-400">Service Level</p>
      <p className="text-xl font-semibold text-red-300">{execOnTimePct.toFixed(1)}%</p>
    </div>

    <div className="rounded-xl border border-orange-900/40 bg-black/20 p-3">
      <p className="text-[11px] uppercase tracking-wide text-slate-400">Demand at Risk</p>
      <p className="text-xl font-semibold text-orange-300">{execLateUnits.toLocaleString()}</p>
    </div>

    <div className="rounded-xl border border-amber-900/40 bg-black/20 p-3">
      <p className="text-[11px] uppercase tracking-wide text-slate-400">Peak Backlog</p>
      <p className="text-xl font-semibold text-amber-300">{execPeakBacklog.toLocaleString()}</p>
    </div>

    <div className="rounded-xl border border-rose-900/40 bg-black/20 p-3">
      <p className="text-[11px] uppercase tracking-wide text-slate-400">Time to Recover</p>
      <p className="text-xl font-semibold text-rose-300">{execTtrDays} days</p>
    </div>
  </div>

  <div className="rounded-xl border border-slate-800 bg-slate-950/45 p-4 mb-4">
    <p className="text-[11px] uppercase tracking-[0.18em] text-slate-400 mb-2">
      Why this matters
    </p>
    <p className="text-sm leading-6 text-slate-200">
      Despite 100% eventual fulfillment, customers experienced delays, backlog accumulated rapidly, and recovery required {execTtrDays} days. The network caught up, but service reliability failed first.
    </p>
  </div>
  ...
  <div className="rounded-xl border border-emerald-900/35 bg-emerald-950/20 p-4">
    ...
    <ul className="space-y-2 text-sm text-slate-200">
      <li>• Prioritize mitigation on the constrained component path.</li>
      <li>• Protect customer-facing service before backlog expands further.</li>
      <li>• Compress recovery time by expediting the limiting supply node.</li>
    </ul>
  </div>
</section>
