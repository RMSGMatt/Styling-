console.log("🔌 AdminPanel LIVE mounted", import.meta?.env?.VITE_API_BASE || `${window.location.origin}/api`);
    ...
    <div className="space-y-6">
      ...
      >
        <p className="text-gray-600">
          Manage users, data, and visibility for your organization. Using{" "}
          <Pill>{API_BASE}</Pill>
        </p>
      </Card>
...
function UsersSection() {
  ...
  useEffect(() => { load(); /* eslint-disable-next-line */ }, [q, limit, offset]);
        ...
        <>
          ...
          <div className="flex justify-between items-center mt-4">
            ...
            <div className="text-sm text-gray-600">
              Offset <Pill>{offset}</Pill> • Limit <Pill>{limit}</Pill>
            </div>
                ...
                {rows.map((r) => (
                  <tr key={r.id} className="border-b last:border-0 align-top">
                    ...
                    <td className="py-2 pr-4 space-y-1">
                      ...
                      {!Object.values(r.outputUrls || {}).some(Boolean) && <span className="text-gray-500">No files.</span>}
                    </td>
                  </tr>
                ))}
              </tbody>
...
function ScenariosSection() {
  const [rows, setRows] = useState([]);
  const [owner, setOwner] = useState(""); // filter owner_email (optional)
  const [loading, setLoading] = useState(true);
  const [err, setErr] = useState("");
